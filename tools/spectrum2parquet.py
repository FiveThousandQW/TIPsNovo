...
def convert_mgf_to_parquet(input_path: str, output_path: str):
    """
    加载一个或多个 MGF 文件，将它们合并，并保存为一个 Parquet 文件。

    Args:
        input_path (str): 输入文件的路径，可以是单个文件或 glob 模式 (例如 "data/*.mgf").
        output_path (str): 输出的 Parquet 文件路径。
    """
    print(f"Loading data from '{input_path}'...")

    # --- 第1步：加载数据 ---
    # 使用 SpectrumDataFrame.load 创建一个对象实例。
    # 我们使用 lazy=False 来确保所有 MGF 文件都被完整读入内存并合并。
    # 注意：这里我们使用原始的 load 方法，它不需要 save_parquet_path 参数。
    sdf = SpectrumDataFrame.load(
        source=input_path,
        lazy=False,
        is_annotated=True,  # 假设您的 MGF 是带注释的
        verbose=True,
    )

    print(f"Successfully loaded {len(sdf):,} spectra into memory.")

    # --- 第2步：保存数据 ---
    # 从 SpectrumDataFrame 实例中获取完整的 Polars DataFrame。
    # to_polars(return_lazy=False) 会返回内存中的 DataFrame。
    full_df = sdf.to_polars(return_lazy=False)

    # 调用 Polars DataFrame 自身的 .write_parquet() 方法来保存文件。
...
if __name__ == "__main__":
    # --- 您需要修改的部分 ---
    input_mgf_path = "/data/qwu/data/TIPnovo/systemMHC/SystemMHC_FineTurn_train_200wPSM_done.mgf"
    output_parquet_path = "/data/qwu/data/TIPnovo/systemMHC/SystemMHC_FineTurn_train_200wPSM.parquet"
    # -------------------------

    print(f"Loading data from {input_mgf_path}...")
    # 使用 SpectrumDataFrame 的加载功能，它会自动进行转换
    # force_convert_to_native=True 确保它执行转换并保存
    convert_mgf_to_parquet(input_mgf_path, output_parquet_path)
