...
from ..constants import ANNOTATED_COLUMN, PROTON_MASS_AMU, MSColumns
from ..types import Peptide, PeptideMask, PrecursorFeatures, Spectrum, SpectrumMask
from ..utils import ResidueSet, SpectrumDataFrame
...
class SpectrumDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[Spectrum, float, int, Peptide | list[str]]:
        # dummy_spectrum = torch.tensor([[1.0, 1.0]], dtype=torch.float32)
        # dummy_precursor_mz = 1.0
        # dummy_precursor_charge = 1
        # dummy_peptide = torch.tensor([1, 2, 3], dtype=torch.long)
        # return dummy_spectrum, dummy_precursor_mz, dummy_precursor_charge, dummy_peptide
        peptide = ""
        ...
        # spectrum = torch.stack([mz_array, int_array]).T.float()
        # # 取前 n_peaks 个，避免后续处理因长度不一而出错
