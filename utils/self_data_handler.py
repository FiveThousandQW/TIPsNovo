...
import random
import re
import shutil # 提供高级文件操作，如此处用于删除临时文件夹
import tempfile # 用于创建临时文件和目录
import time # 用于生成全局唯一ID (UUID)，这里用来创建唯一的临时文件名
import uuid
from concurrent.futures import ThreadPoolExecutor
from enum import Enum # 用于创建枚举类型
from itertools import chain
from pathlib import Path
from typing import TYPE_CHECKING, Any, Callable, Iterator, Union, cast  # 提供类型提示支持
